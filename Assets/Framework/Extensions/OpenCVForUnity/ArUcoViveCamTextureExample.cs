...
namespace OpenCVForUnityExample
{
    ...
    /// </summary>
    [RequireComponent (typeof(ViveCamToMatHelper))]
    public class ArUcoViveCamTextureExample : MonoBehaviour
    {
        ...
        /// </summary>
        ViveCamToMatHelper viveCamToMatHelper;
        ...
        void Start ()
        {
            ...
            viveCamToMatHelper = gameObject.GetComponent<ViveCamToMatHelper> ();
            viveCamToMatHelper.Initialize();
        }
        ...
        public void OnWebCamTextureToMatHelperInitialized()
        {
            ...
            Mat viveCamTextureMat = viveCamToMatHelper.GetMat();

            texture = new Texture2D(viveCamTextureMat.cols(), viveCamTextureMat.rows(), TextureFormat.RGB24, false);
            Utils.fastMatToTexture2D(viveCamTextureMat, texture);

            //gameObject.GetComponent<Renderer>().material.mainTexture = texture;

            gameObject.transform.localScale = new Vector3(viveCamTextureMat.cols(), viveCamTextureMat.rows(), 1);
            Debug.Log("Screen.width " + Screen.width + " Screen.height " + Screen.height + " Screen.orientation " + Screen.orientation);

            if (fpsMonitor != null) {
                fpsMonitor.Add("width", viveCamTextureMat.width().ToString());
                fpsMonitor.Add("height", viveCamTextureMat.height().ToString());
                fpsMonitor.Add("orientation", Screen.orientation.ToString());
            }


            float width = viveCamTextureMat.width();
            float height = viveCamTextureMat.height();
            ...
            else if (useViveCameraParameters)
            {
                CameraParameters param;
                param = viveCamToMatHelper.GetCameraParameters();
                distCoeffs = new MatOfDouble(param.GetDistortionCoefficients());
                camMatrix = param.GetCameraMatrix();
                fx = param.camera_matrix[0];
                fy = param.camera_matrix[4];
            ...
            rgbMat = new Mat(viveCamTextureMat.rows(), viveCamTextureMat.cols(), CvType.CV_8UC3);
            ids = new Mat();
            ...
            Debug.Log(detectorParams.get_minMarkerPerimeterRate());
            detectorParams.set_minMarkerPerimeterRate(0.002d);
            detectorParams.set_minCornerDistanceRate(0.002d);
            detectorParams.set_errorCorrectionRate(3.0d);
            detectorParams.set_maxErroneousBitsInBorderRate(0.3f);
            dictionary = Aruco.getPredefinedDictionary((int)dictionaryId);

            dictionary = Aruco.custom_dictionary(8, 3);
            //gridBoard = GridBoard.create(2, 4, 0.024f, 0.006f, dictionary);
            //gridBoard = GridBoard.create(1, 2, 0.045f, 0.006f, dictionary);
            gridBoard = GridBoard.create(1, 3, 0.035f, 0.006f, dictionary);



            charucoBoard = CharucoBoard.create(2, 4, 0.0315f, 0.024f, dictionary);

            //Mat marker = new Mat(512, 256, CvType.CV_8UC3);
            //gridBoard.draw(new Size(256, 512), marker, 16);
            //markerTexture = new Texture2D(256, 512, TextureFormat.RGB24, false);
            //Utils.matToTexture2D(marker, markerTexture);
            //byte[] bytes = markerTexture.EncodeToPNG();
            //var dirPath = Application.dataPath + "/../";
            //File.WriteAllBytes(dirPath + "Marker_grid_1x3" + ".png", bytes);
        ...
        void Update ()
        {            
            if (viveCamToMatHelper.IsPlaying () && viveCamToMatHelper.DidUpdateThisFrame ()) {
                
                Mat rgbaMat = viveCamToMatHelper.GetMat ();
...
//                Imgproc.putText (rgbaMat, "W:" + rgbaMat.width () + " H:" + rgbaMat.height () + " SO:" + Screen.orientation, new Point (5, rgbaMat.rows () - 10), Imgproc.FONT_HERSHEY_SIMPLEX, 1.0, new Scalar (255, 255, 255, 255), 2, Imgproc.LINE_AA, false);
                ...
                //leftEye.mainTexture = texture;
            }
        }

        public Material leftEye;
        public bool useViveCameraParameters;
        ...
        public void OnMarkerTypeDropdownValueChanged (int result)
        {
            ...
            //    if (viveCamToMatHelper.IsInitialized ())
            //        viveCamToMatHelper.Initialize ();
            //}
        }
        ...
        public void OnUseStoredCameraParametersToggleValueChanged()
        {
            ...
            if (viveCamToMatHelper != null && viveCamToMatHelper.IsInitialized())
            {
                viveCamToMatHelper.Initialize();
            }
        }
