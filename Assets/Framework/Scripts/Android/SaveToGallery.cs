...
public class SaveToGallery : MonoBehaviour
{
    ...
    public static string SaveImageToGallery(byte[] data, string title, string description)
    {
        using (var mediaClass = new AndroidJavaClass(MediaStoreImagesMediaClass))
        {
            using (var cr = Activity.Call<AndroidJavaObject>("getContentResolver"))
            {
                var image = ByteArrayToAndroidBitmap(data);
                var imageUrl = mediaClass.CallStatic<string>("insertImage", cr, image, title, description);
                return imageUrl;
            }
        }
    ...
    public static AndroidJavaObject ByteArrayToAndroidBitmap(byte[] encoded)
    {
