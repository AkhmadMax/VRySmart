...
namespace Google.XR.ARCoreExtensions.Samples.CloudAnchors
{
    ...
    /// </summary>
    public class CustomCloudAnchorsExampleController : MonoBehaviour
    {
...
#pragma warning restore 618
        ...
        private void MessagesHandler_OnCommandEvent(int netMsg)
        {
            ToggleMarkerScreen();
        }
        ...
        void ToggleMarkerScreen()
        {
            if(_currentActiveScreen != ActiveScreen.MarkerScreen)
            {
                tempScreen = _currentActiveScreen;
                SwitchActiveScreen(ActiveScreen.MarkerScreen);
            }
            else
            {
                SwitchActiveScreen(tempScreen);
            }
        }
